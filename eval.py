...
matplotlib.style.use('ggplot')  # env = gym.envs.make("MountainCarContinuous-v0")
...
# saver = tf.train.import_meta_graph('model/100-950.ckpt.meta')
sess = tf.Session()
saver = tf.train.Saver()
saver.restore(sess, 'model/100-950.ckpt.meta')
value_estimator = ValueEstimator(scaler, featurizer, learning_rate=0.1)
state = np.array([0, 0, 50])
value_next = value_estimator.predict(state, sess)
print value_next
